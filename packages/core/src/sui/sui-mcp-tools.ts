...
export function getSuiMCPTools(): Tool[] {
  return [
    ...
    {
      name: 'cetus_swap',
      description: `在 Cetus DEX 上执行代币兑换（Swap）。
支持任意两种代币之间的兑换，自动路由最优池子。
示例：在 Cetus 上卖出 SUI 买入 USDC，或交换 USDC 为 CETUS。

参数说明：
- coinTypeIn: 输入代币的完整类型（如 "0x2::sui::SUI"）
- coinTypeOut: 输出代币的完整类型（如 "0x...::usdc::USDC"）
...
- poolId: 可选，指定交易池 ID`,
      inputSchema: {
        ...
        properties: {
          coinTypeIn: {
            type: 'string',
            description: '输入代币的完整类型，如 "0x2::sui::SUI"',
          },
          coinTypeOut: {
            type: 'string',
            description: '输出代币的完整类型，如 "0x...::usdc::USDC"',
          },
          amount: {
            type: 'string',
            description: '输入金额（最小单位），如 "1000000000" = 1 SUI',
          },
          byAmountIn: {
            type: 'boolean',
          ...
          network: {
            type: 'string',
            description: '网络，可选 "mainnet" 或 "testnet"，默认 "testnet"',
            default: 'testnet',
          },
        },
    ...
    {
      name: 'cetus_get_quote',
      description: `获取 Cetus DEX 上的兑换报价。
在执行 swap 前先获取报价，了解预期兑换率和滑点。
...
- byAmountIn: true=固定输入, false=固定输出`,
    ...
    },
    {
      ...
      description: `获取 Cetus DEX 上可用的交易池列表。
可用于查询特定代币对的可用池子。
...
- coinTypeB: 可选，代币 B 的类型`,
    ...
    {
      name: 'navi_deposit',
      description: `在 Navi Protocol 上存入资产。
...
- amount: 存入金额（最小单位）`,
    ...
    {
      name: 'navi_withdraw',
      description: `从 Navi Protocol 提取资产。
提取之前存入的资产。
...
- amount: 提取金额（最小单位）`,
    ...
    {
      name: 'navi_borrow',
      description: `从 Navi Protocol 借出资产。
使用已存入的资产作为抵押，借出指定代币。
...
- amount: 借出金额（最小单位）`,
    ...
    {
      name: 'navi_repay',
      description: `偿还 Navi Protocol 的借款。
归还之前借出的资产。
...
- amount: 偿还金额（最小单位）`,
    ...
    {
      name: 'sui_transfer',
      description: `在 Sui 区块链上转账 SUI 或代币。
将指定代币发送到目标地址。
